---
name: authoring
description: "Author NEW skills, agents, memory & tools from signal (Scribe office) — scan a transcript for reusable PROCEDUREs (skill drafts), recurring DELEGATION needs (agent drafts), and durable FACTs (memory drafts), and scan the observation log for recurring ad-hoc scripts (tool candidates), staging all to staged/ then adopting autonomously (agent judges quality; git is the safety net). Run when you want the agent to grow its own capabilities from what it keeps doing. Triggers on '/authoring', 'author a new skill', 'author an agent', 'author a tool', 'author a memory', '從 session 長出 skill', '把這套流程變成 skill', '把這個重複的 script 變成 tool', 'scribe authoring', 'grow a new worker / tool', 'kilo 自己長 skill/agent/tool'."
---

# Authoring — grow new skills, agents, memory & tools (Scribe office)

從 session signal 長出**新**手稿:反覆出現、長期需要的 procedure → skill draft;反覆被下放的工作 → agent draft;耐久的事實 / 慣例 / 外部參照 → memory draft。候選先落 `staged/author.jsonl`(品質中繼,萃取與採納分兩步),**採納由 agent 自主判斷執行,不等 Loki 點頭**(2026-07-09 裁決:成長閉環全自動,見 memory `feedback_kilo_growth_is_agents_job`;安全網 = git 歷史 + 回報透明)。
...
- 不值得的 → 刪掉那行。**採納是 agent 逐條的品質判斷,不是無腦全收** —— 判準仍是「如果會長期需要的話」;收了什麼、丟了什麼,收工時回報。
...
- **採納(agent 自主)** → 派 apply agent(instance 的 `utils-promoter`,或等價)把候選變成 `<tools-repo>/scripts/<slug>` + 開 PR。**engine 不碰外部 repo / 不開 PR**(repo-specific 留 instance)。
- 對稱:`tool_author`(創) ↔ `corrector/tool_review`(校,讀 utils-usage 失敗率)。兩者都不自動改 tool repo。

> 完整 tool 自循環:`observe`(記 ad-hoc + usage)→ `tool_author`(創候選)/ `tool_review`(校既有)→ agent 採納 → apply agent 寫 script + PR → shim 改即 live。取代了手動 `/review` cluster;apply agent 是保留的執行層,不是被取代對象。

## 邊界

- **萃取器不落地**:`author.py`(script)永不寫 `skills/` `agents/` `CANON.md` —— staged 是它的唯一輸出;採納寫入由 agent 在 session 中判斷執行(全自動指的是不等 Loki,不是 script 直寫)。
