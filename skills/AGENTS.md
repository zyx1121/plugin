...
`description` 是 Claude 選 skill 的唯一觸發面:routing 對 `name`+`description` 做 keyword/intent 比對。寫法不一致不影響能不能載,但影響能不能穩定 route,也增加維護 diff 噪音。

## Grammar(硬規範)

1. 整個 value 用雙引號 YAML scalar 包:`description: "..."`。不用裸 scalar、不用單引號 YAML。
2. Trigger 字串用單引號 `'...'`,外層雙引號就免 escape。
3. Trigger list 分隔 = ASCII `, `(逗號 + 空格)。中文 prose 內的全形 `，。、：` 維持全形;規範只管 `Triggers on` 之後、引號字串之間的分隔符。
