...
本檔按目的分層,讀的順序 = 做的順序:**§1 先分類(目的決定規範)→ §2 共用底線(官方 MUST)→ §3A 報告 / §3B 教學 / §3C 分鏡 專章(擇一)→ §4 落地引擎(共用)→ §5 Self-review**。內容先於工具 — 規範定了才進落地章。沒有外部 skill 依賴。
...
動手前先定這份 deck 是哪一類 —— 三類的內容規範(密度 / 語言 / 標題 / 字級 / 節奏)直接衝突,選錯整份走鐘。定了就只讀對應的專章(§3A / §3B / §3C);落地引擎(§4)共用。

| | 報告 deck(技術簡報) | 教學 deck(教學簡報) | 分鏡 deck(工作坊簡報) |
|---|---|---|---|
| 場景 | lab talk / pitch / demo / 現場報告 | 錄課 / 線上課程影片 / MOOC | 現場工作坊 / hands-on 課程(邊講邊操作) |
| 語言 | 投影片**英文** | 投影片**中文**(面向學生) | **英文短標題 + 中文一句 caption** |
| 密度 | **高** — nested bullets 塞滿 | **低** — 一片一重點、≤6 行 | **極低** — 一頁一 beat、一句話 |
| 標題 | claim / dash 句型 | 知識點名稱即可 | 英文短語,是節奏器不是內容 |
| 頁數 | 正常 | 正常 | 傳統的 **3–4 倍**(翻頁即動畫) |
| 專章 | §3A | §3B | §3C |

教學 vs 分鏡的分界:**錄影自播 = 教學(§3B),現場帶操作 = 分鏡(§3C)**。教學 deck 一頁要撐 30–60 秒口白;分鏡 deck 一頁只撐 ≤15 秒,節奏由翻頁製造。
...
> **刻意偏離官方**(其餘照守):
> 1. 官方 SHOULD「每 bullet ≤1 行」是單一通用密度;我們按 deck 類別分 —— 報告 deck 高密度 nested(撐不過一行才拆下一層)、教學 deck ≤6 行、分鏡 deck 一句話。
> 2. 分鏡 deck(§3C)刻意違反「標題唯一」與「One topic, one slide」—— 同圖差分會讓同標題 / 同主題跨數十頁,那是節奏設計不是冗餘。報告 / 教學 deck 仍必守這兩條。
...
## §3C 分鏡 deck(工作坊簡報)— 內容規範

給**現場 hands-on 工作坊 / live 課程**用:講者在場、學員邊聽邊操作。核心信念:**翻頁本身就是動畫,頁數不是成本,單頁停留時間才是**。一頁一個 beat、單頁 ≤15 秒,總頁數是傳統 deck 的 3–4 倍(225 頁 ≈ 傳統 60 頁的內容量)。師承 Lessig Method(投影片如字幕)、高橋メソッド(整頁一句大字)、assertion-evidence(標題主張 + 圖為證)、Duarte 的 progressive disclosure(同圖漸進揭露)。黃金標準 = `Claude Code CLI 理念與實作`(MTK 課程 deck,source 在 iCloud MTK 課程專案)。

### 版式契約(每頁固定三段)

- **上:英文短標題**(特大、粗黑)— 是節奏器不是內容;短語即可(`Assemble the Context` / `Too Bad!`),不用完整 claim 句
- **中:一張圖** — 架構差分圖 / terminal 截圖 / 迷因,擇一;沒圖的純文字 beat 頁就置中 1–3 短句
- **下:中文一句 caption** — **這行才是 deck 真正的 script**:抽掉講者後靠它自讀。每頁必寫、只寫一句、關鍵詞上色

### 視覺語意系統(三條,全 deck 一致)

1. **雙色語意**:藍 = 已知 / 背景,橘 = 當前焦點 / 新登場。每翻一頁**只有橘色的位置在動**,觀眾視線被顏色牽引。caption 內的關鍵詞同步用橘。
2. **虛線 = 容器邊界**(Claude Code、Context),實心圓角框 = 元件。與 §4 diagram 引擎的 zone 語意一致。
3. **概念 vs 實況分離**:概念用白底手繪風方塊圖,實況用原色深底 terminal 截圖 —— 兩種頁面交替出現,學員一眼分得出「這是模型」還是「這是真的畫面」。

### 節奏機制(五個,組合使用)

1. **一頁一動作** — 把 build 動畫拆成獨立頁。一頁只推進一件事:一個元件登場、一條線亮起、一個問題拋出。塞兩件事就拆兩頁。
2. **同圖差分** — 一張底圖跨數十頁,只換 highlight 與周邊小標籤。**底圖元素位置永不移動**(觀眾的空間記憶是資產),要加新元件就在最初版就留好位。先鎖定底圖、再開始複製頁,否則改一處要回頭改幾十頁。
3. **提問頁當鉤子** — 每 3–5 頁插一頁純提問(`What's in Context?` / `Why This Time?`),先製造認知缺口,之後逐頁填。答案永遠不跟問題同頁。
4. **停頓點標時間** — `Hands-on Time` 頁必標分鐘數(5 / 15 / 25 min),`Break` 頁同。學員的操作指令逐字放在頁上(可直接照打),不是口頭轉述。
5. **模板化重複** — 重複性段落(如多個 checkpoint)走固定模板:痛點 → Good Idea → Goal → Steps → 驗證。第二輪起學員能預測結構,注意力全留給內容差異。

### 敘事邏輯(四條)

1. **現象先於術語** — 先讓學員看到行為(說嗨、它記得我),再問「裡面是什麼」,術語是被問出來的,不是被定義出來的。每個新概念由上一層的殘留疑問驅動(剝洋蔥),沒有概念憑空空降。
2. **單一 running example 貫穿** — 一個例子跑完基礎迴圈後,每個新機制都回到**同一張迴圈圖**重走一遍,用重複建立肌肉記憶。比喻域儘量與練習域雙關(用圖書館比喻 Skills,練習題就是圖書館系統)。
3. **地圖圖開場、結尾合體** — 全 deck 的骨架用一張中心輻射地圖呈現;每講完一個機制,回地圖補上一個四字定位(常駐指令 / 按需知識 / 外部能力…)。結尾把補滿的地圖原圖再放一次 = 總結頁零成本。
4. **收在能力進化,不是知識清單** — 結尾用第二人稱寫學員獲得了什麼能力(「你已經不用一句一句交代了」),不是重列教了哪些名詞。

### 迷因規範

迷因是**情緒標點**,只放在「疑問」與「失望 / 轉折」的 beat 頁,一份 deck ≤5 張,不當裝飾、不進技術圖。判準:把迷因抽掉,該頁必須還成立(它只加情緒不載資訊)。

### 維護代價(進場前想清楚)

- 同圖差分 = 一張圖散在幾十頁;**底圖沒鎖定前不要開始複製頁**,改版時用「找到所有含此圖的頁」清單逐頁改
- 這種 deck 幾乎不可轉印講義;要交講義另出 §3B 式濃縮版,別直接印 225 頁

### 落地限制(現行 builder 不支援,誠實列出)

分鏡 deck 目前 **builder 產不出來**,三個缺口:(a) 無「大標 + 置中圖 + 底部 caption」版式;(b) diagram 引擎鎖白底黑字,無 per-box 顏色 override,做不了藍 / 橘差分;(c) caption 關鍵詞 inline 上色未開放。現行做法 = 本章當**內容與分鏡規範**用(outline / 每頁 beat / caption 全文照本章產出),落地進 Keynote / PowerPoint 手做;或擴 builder(TODO:`beat` layout + box `color` override + inline run 上色)。

## §4 落地引擎(報告 / 教學共用;分鏡 deck 見 §3C 落地限制)
...
- [ ] 每頁一個 beat(一頁只推進一件事,口白 ≤15 秒)
- [ ] 每頁有中文一句 caption,連起來能當 script 自讀
- [ ] 雙色語意一致:藍 = 已知、橘 = 焦點;每頁只有橘在動
- [ ] 差分序列的底圖元素位置全程沒動過
- [ ] 提問頁與答案頁分開;每 3–5 頁有一個鉤子
- [ ] Hands-on / Break 頁標了分鐘數,操作指令逐字在頁上
- [ ] 迷因 ≤5 張且抽掉不影響內容
- [ ] 地圖圖:開場出現、逐段補標籤、結尾合體
- [ ] 結尾收在學員的能力進化,不是名詞清單
