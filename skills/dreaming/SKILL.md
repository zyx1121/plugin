...
- 跑 `python3 "${CLAUDE_PLUGIN_ROOT}/armarium/gen_memory_index.py" "$SCRIPTORIUM_HOME/memory"` —— 看輸出的 `LINT` 區塊,它列出 bad-type / orphan-link / unquoted-desc 三類規範問題,即「規範對齊」step 的待修清單。
