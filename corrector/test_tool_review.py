...
class AggregateTest(unittest.TestCase):
    ...
    def test_benign_cwd_reset_stderr_not_a_failure(self):
        # M1: mac-app 2/6 runs flagged failed on this alone — harness notice,
        # not an atom error.
        recs = [
            {"kind": "utils-usage", "script": "mac-app", "interrupted": False,
             "stderr_tail": "\nShell cwd was reset to /Users/dev"},
            {"kind": "utils-usage", "script": "mac-app", "interrupted": False,
             "stderr_tail": "\nShell cwd was reset to /Users/dev"},
            {"kind": "utils-usage", "script": "mac-app", "interrupted": False, "stderr_tail": ""},
        ]
        stats = tr.aggregate(recs)
        self.assertEqual(stats["mac-app"], {"calls": 3, "failures": 0})

    def test_real_error_alongside_benign_notice_still_counts(self):
        recs = [{"kind": "utils-usage", "script": "mac-app", "interrupted": False,
                  "stderr_tail": "traceback: boom\nShell cwd was reset to /Users/dev"}]
        stats = tr.aggregate(recs)
        self.assertEqual(stats["mac-app"], {"calls": 1, "failures": 1})

    def test_mcp_transport_records_aggregate_via_script_field(self):
        # Shape as produced by scribe/observe.py's _build_mcp_record: `script`
        # mirrors the resolved `atom` specifically so this needs zero changes
        # here to pick up MCP usage alongside the Bash-CLI path.
        recs = [
            {"kind": "utils-usage", "transport": "mcp", "tool": "pve_list", "atom": "pve",
             "script": "pve", "params": {}, "interrupted": False, "stderr_tail": ""},
            {"kind": "utils-usage", "transport": "mcp", "tool": "pve_stop", "atom": "pve",
             "script": "pve", "params": {"name": "x"}, "interrupted": False,
             "stderr_tail": "file not found"},
            # Bash-CLI call to the same underlying atom -> same bucket.
            {"kind": "utils-usage", "script": "pve", "command": "utils pve list",
             "interrupted": False, "stderr_tail": ""},
            # unresolved atom (script also null, per the MCP branch's contract) -> ignored.
            {"kind": "utils-usage", "transport": "mcp", "tool": "totally_unknown", "atom": None,
             "script": None, "params": {}, "interrupted": False, "stderr_tail": ""},
        ]
        stats = tr.aggregate(recs)
        self.assertEqual(stats["pve"], {"calls": 3, "failures": 1})
        self.assertNotIn(None, stats)


class IsBenignStderrTest(unittest.TestCase):
    def test_pure_cwd_reset_is_benign(self):
        self.assertTrue(tr._is_benign_stderr("\nShell cwd was reset to /Users/dev"))
        self.assertTrue(tr._is_benign_stderr("Shell cwd was reset to /Users/dev\n"))

    def test_real_error_is_not_benign(self):
        self.assertFalse(tr._is_benign_stderr("Traceback (most recent call last):\n..."))

    def test_empty_is_not_benign(self):
        self.assertFalse(tr._is_benign_stderr(""))
        self.assertFalse(tr._is_benign_stderr("   "))

    def test_mixed_content_is_not_benign(self):
        self.assertFalse(tr._is_benign_stderr("error: boom\nShell cwd was reset to /Users/dev"))
