...
# The harness's own bookkeeping notice, not an atom error — observe.py has been
# seen to record this as the entirety of stderr_tail (M1: 2/6 mac-app runs
# flagged failed on this alone). Matched per-line so a genuine error mixed with
# this notice still counts as a failure.
_BENIGN_STDERR_RE = re.compile(r"^Shell cwd was reset to .+$")


def _is_benign_stderr(stderr_tail: str) -> bool:
    """True when every non-empty line of stderr_tail is just the harness's
    cwd-reset notice — i.e. not a real atom failure signal."""
    lines = [ln.strip() for ln in stderr_tail.splitlines() if ln.strip()]
    return bool(lines) and all(_BENIGN_STDERR_RE.match(ln) for ln in lines)
...
def aggregate(records: list[dict]) -> dict[str, dict[str, int]]:
    ...
    for r in records:
        ...
        if r.get("interrupted") or (stderr_tail and not _is_benign_stderr(stderr_tail)):
            s["failures"] += 1
    return stats
