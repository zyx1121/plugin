...
export const timetableTools: ToolboxTool[] = [
  ...
  scriptTool({
    ...
    description: "Look up class meeting time (day/period/room) for specific course IDs. cos_id is the part after the dot in an E3 shortname like 1151.535702 (acysem is the part before it) — chain directly off e3p_list_courses output. Unmatched IDs are reported, not treated as an error.",
    inputSchema: {
      cos_ids: z.array(z.union([z.string(), z.number()])).describe("One or more course IDs (cos_id), e.g. 535702."),
      acysem: z.string().optional().describe("Semester code like 1151. Default: latest semester."),
    },
    annotations: read,
    script,
