...
class BuildRowsTest(unittest.TestCase):
    def test_rows_sorted_and_no_title_uses_bare_link(self):
        with TemporaryDirectory() as d:
            ...
            self.assertEqual(rows[2], "- feedback_c.md — no title")          # bare link, no title bracket
...
class GenIndexCliTest(unittest.TestCase):
    def test_writes_index_file(self):
        with TemporaryDirectory() as d:
            ...
            self.assertEqual(idx.splitlines()[0], gmi.hot_header(mem))       # cold-index pointer, line 1
            self.assertIn(str((mem / "MEMORY-COLD.md").resolve()), idx.splitlines()[0])  # points at THIS dir
            self.assertIn("- [X](x.md) — the x", idx)
            self.assertTrue((mem / "MEMORY-COLD.md").is_file())              # always written alongside
...
class MemorySyncSmokeTest(unittest.TestCase):
    ...
    def test_common_memory_merged_into_index(self):
        # a mounted common-memory/memory submodule is merged into the one index, links relative.
        # type=reference tiers to MEMORY-COLD.md (not MEMORY.md) — see gen_memory_index tiering.
        ...
        r = self._run_sync()
        self.assertEqual(r.returncode, 0)
        hot = (Path(self.home) / "memory" / "MEMORY.md").read_text()
        cold = (Path(self.home) / "memory" / "MEMORY-COLD.md").read_text()
        self.assertIn("- [Foo](foo.md) — a foo", hot)                                        # private (bare), hot
        self.assertIn("Dev](../common-memory/memory/reference_dev.md) — shared fact", cold)  # common (relative), cold
