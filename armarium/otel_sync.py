...
def collect_utils_usage(hwm: str) -> tuple[list[dict], str]:
    ...
    # Aggregate by (session, script). A falsy `script` (missing, or explicitly
    # None — an MCP record with an unresolved atom) is skipped rather than
    # falling through to a nonexistent `command` key, which used to produce a
    # bogus empty-string bucket. Matches tool_review.aggregate()'s identical
    # skip behavior for the same case.
