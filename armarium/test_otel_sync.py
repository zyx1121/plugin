...
class UtilsUsageMcpNullScriptTest(unittest.TestCase):
    """An MCP record with an unresolved atom carries `script: None` (scribe/
    observe.py's contract — script mirrors atom, null when atom is null).
    Must be skipped like tool_review.aggregate() skips it, not folded into a
    bogus empty-string bucket via the old `command`-key fallback."""

    _ENTRIES = [
        {"ts": "2026-06-01T10:00:00+00:00", "session": "s1", "kind": "utils-usage",
         "transport": "mcp", "tool": "totally_unknown", "atom": None, "script": None,
         "params": {}, "interrupted": False, "stderr_tail": ""},
        {"ts": "2026-06-01T10:01:00+00:00", "session": "s1", "kind": "utils-usage",
         "transport": "mcp", "tool": "pve_list", "atom": "pve", "script": "pve",
         "params": {}, "interrupted": False, "stderr_tail": ""},
    ]
    ...
    def test_null_script_excluded_resolved_atom_still_aggregates(self):
        with patch.object(otel_sync.paths, "data_dir", return_value=self.home / "data"):
            records, _ = otel_sync.collect_utils_usage("")
        # Only the resolved-atom record produces a bucket; the null-script
        # record contributes neither its own record nor a "" bucket.
        self.assertEqual(len(records), 1)
        self.assertEqual(records[0]["body"]["stringValue"], "utils-usage: pve calls=1 fails=0")
        bodies = " ".join(r["body"]["stringValue"] for r in records)
